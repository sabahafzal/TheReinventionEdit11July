// config/challengesLibrary.js
//
// ─── HOW THE ROTATION WORKS ───────────────────────────────────────────────────
//
// Each challenge has a `slot` (1–4). Only ONE challenge per slot is active at
// any given time per circle. When a challenge expires, the next one in that
// slot (by `order`) is seeded automatically by the scheduleNextChallenge()
// helper (called from a Supabase Edge Function or cron).
//
// This means a circle shows a maximum of 4 challenges at once — one per slot —
// and the library provides a long queue of content behind each slot.
//
// Slot guide (consistent across all roadmaps):
//   Slot 1 → Short burst  (1–3 days) — quick wins
//   Slot 2 → Weekly habit (7 days)   — consistency builder
//   Slot 3 → Deep focus   (7 days)   — skill or mindset
//   Slot 4 → Social       (7 days)   — community / accountability
//
// ─────────────────────────────────────────────────────────────────────────────
...
// ─── Helper: get the first challenge for a given roadmap + slot ───────────────
// Used when seeding a brand new circle.
export function getInitialChallenges(roadmapKey) {
  const all = CHALLENGES_LIBRARY[roadmapKey] || [];
  const slots = {};
  for (const ch of all) {
    if (!slots[ch.slot] || ch.order < slots[ch.slot].order) {
      slots[ch.slot] = ch;
    }
  }
  return Object.values(slots); // returns one challenge per slot
}

// ─── Helper: get the next challenge for a given roadmap + slot + current order
// Used by scheduleNextChallenge() to rotate when a challenge expires.
export function getNextChallenge(roadmapKey, slot, currentOrder) {
  const all = CHALLENGES_LIBRARY[roadmapKey] || [];
  const inSlot = all
    .filter((ch) => ch.slot === slot)
    .sort((a, b) => a.order - b.order);

  // Find next order after currentOrder; wrap around to order 1 if at end
  const next = inSlot.find((ch) => ch.order > currentOrder);
  return next || inSlot[0] || null;
}